...
import { propertyFromFirestore, type ListingPurpose, type PropertyStatus } from "../../lib/types";
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      try {
        ...
        setPrice(property.price ? String(property.price) : ""); setPropertyType(property.propertyType || "lakás"); setListingPurpose(property.listingPurpose);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updateDoc(doc(db, "posts", params.id as string), {
        ...
        propertyType, listingPurpose, area: Number(area), rooms: Number(rooms), condition: condition.trim(),
        floor: floor.trim(), balcony: balcony.trim(), parking: parking.trim(), heating: heating.trim(),
        status, description: description.trim(), phone: normalizeHungarianPhone(phone) || "", email: email.trim(),
    ...
    <div className="flex min-h-screen justify-center bg-[#f7f4ee] p-6 text-[#18201b]">
      <div className="w-full max-w-3xl rounded-[32px] border border-[#e2ddd3] bg-white p-8 shadow-[0_24px_70px_rgba(55,47,33,.10)]">
        ...
        <div className="mb-4 grid gap-4 md:grid-cols-3">
          <div><label className="mb-2 block text-[#6c776f]">Hirdetés típusa</label><select value={listingPurpose} onChange={(e) => setListingPurpose(e.target.value as ListingPurpose)} className={inputClass}><option value="sale">Eladó</option><option value="rent">Kiadó</option></select></div>
