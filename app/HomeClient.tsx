...
export default function HomeClient({ initialPosts }: { initialPosts: PropertyWithId[] }) {
  ...
  useEffect(() => {
    ...
    const fetchPosts = async () => {
      try {
        ...
        setPosts(snapshot.docs.map((item) => propertyFromFirestore(item.id, item.data() as Record<string, unknown>)));
      } catch (error) {
        console.error("Hiba az aktív ingatlanok betöltésénél:", error);
      } finally {
        setLoading(false);
      }
    };
